		...
		<div v-else class="dashboard-content">
			...
			<div class="section accounts-section">
				...
				<div v-if="accountsStore.hasAccounts" class="accounts-grid">
					<div v-for="account in accountsStore.allAccounts" :key="account.mapping.wowAccountId" class="account-card">
						<div class="account-card-header">
							...
							<button v-if="authConfig.accountLinkingEnabled" @click.stop="handleUnlinkAccount(account)"
								class="unlink-button" title="Unlink account">
								✕
							</button>
						</div>

						<div class="account-card-body" @click="selectAccount(account.mapping.wowAccountId)">
				...
				<div v-else class="no-accounts">
					<p v-if="authConfig.isDirectAuth">No account found. Please log in again or contact support.</p>
					<p v-else>No WoW accounts linked yet. Link your first account to get started!</p>
				</div>
			</div>
			...
			<div v-if="!authConfig.isDirectAuth" class="section create-account-section">
				...
				<p class="section-description">Don't have a WoW account yet? Create one here and link it to your profile.</p>
...
const onAccountCreated = async (credentials: { username: string; password: string }) => {
	...
	if (userId.value) {
		try {
			await accountsStore.createAccountMapping(userId.value, credentials.username, credentials.password);
			await accountsStore.loadAccounts(userId.value);
		} catch (err) {
...
const handleUnlinkAccount = async (account: ManagedAccount) => {
	if (
		!confirm(
			`Are you sure you want to unlink "${account.mapping.wowAccountName}"? This will remove the connection between your web account and this WoW account.`,
		)
	) {
		return;
	}
